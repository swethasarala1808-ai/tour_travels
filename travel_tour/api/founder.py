...
def get_founder_data():
    ...
    supplier_contracts = frappe.get_all('Supplier Contract',
        fields=['name','supplier','supplier_name','contract_start_date',
                'contract_end_date','cost_per_pax','currency','notes','creation'],
        order_by='creation desc', limit=200)
    ...
    open_leads = len([l for l in leads if l.get('status') in ['Open','Interested','Contacted']])
    pending_visas = len([v for v in visa_apps if v.get('status') in ['Pending Documents','Documents Collected','Submitted']])
    ...
    return {
        ...
        'stats': {
            'total_revenue': total_revenue, 'active_bookings': len(bookings),
            'open_leads': open_leads, 'lead_count': len(leads),
            'package_count': len(packages), 'visa_count': len(visa_apps),
            'pending_visas': pending_visas,
            'destination_count': len(destinations), 'agent_count': len(visa_agents),
        },
        'founder': {'name': founder_name, 'email': frappe.session.user, 'role': 'Founder & CEO'}
    }
...
def save_lead(**kwargs):
    ...
    for f in ['full_name','email_id','mobile_no','status','source','suggested_package','pax_count','preferred_month','assigned_consultant','interest_tags','remarks']:
        if kwargs.get(f) is not None: setattr(doc, f, kwargs[f])
...
def save_booking(**kwargs):
    ...
    for f in ['customer','customer_mobile','tour_package','departure_date','total_pax','sales_consultant']:
        if kwargs.get(f) is not None: setattr(doc, f, kwargs[f])
    doc.flags.ignore_mandatory = True
    doc.save(ignore_permissions=True); frappe.db.commit()
    return {'success': True, 'name': doc.name}
...
def update_booking_status(booking_name, status):
    if frappe.db.exists('Booking', booking_name):
        frappe.db.set_value('Booking', booking_name, 'docstatus', {'Draft':0,'Confirmed':1,'Cancelled':2}.get(status,0))
...
def save_visa(**kwargs):
    ...
    for f in ['booking','applicant_name','passport_number','passport_expiry','destination_country','visa_type','departure_date','status','submission_deadline','all_docs_collected']:
        if kwargs.get(f) is not None: setattr(doc, f, kwargs[f])
    doc.save(ignore_permissions=True); frappe.db.commit()
    return {'success': True, 'name': doc.name}
...
def save_supplier_contract(**kwargs):
    ...
    for f in ['supplier','contract_start_date','contract_end_date','cost_per_pax','currency','notes']:
        if kwargs.get(f) is not None: setattr(doc, f, kwargs[f])
    doc.flags.ignore_mandatory = True
    doc.save(ignore_permissions=True); frappe.db.commit()
    return {'success': True, 'name': doc.name}
...
def save_settings(**kwargs):
    try:
        doc = frappe.get_doc('Travel Tour Settings', 'Travel Tour Settings') if frappe.db.exists('Travel Tour Settings', 'Travel Tour Settings') else frappe.new_doc('Travel Tour Settings')
        for f in ['ops_email','site_url','razorpay_key_id','consultant_commission_pct','website_booking_bonus']:
            if kwargs.get(f) is not None: setattr(doc, f, kwargs[f])
        doc.save(ignore_permissions=True); frappe.db.commit()
        return {'success': True}
...
def save_visa_delivery(**kwargs):
    ...
    for f in ['visa_application','applicant_name','delivery_date','delivery_mode','tracking_number','notes']:
        if kwargs.get(f) is not None: setattr(doc, f, kwargs[f])
    doc.save(ignore_permissions=True); frappe.db.commit()
    return {'success': True, 'name': doc.name}
...
def save_visa_fee_billing(**kwargs):
    ...
    for f in ['customer','booking','visa_application','embassy_fee','service_charge','gst_amount','grand_total']:
        if kwargs.get(f) is not None: setattr(doc, f, kwargs[f])
    doc.flags.ignore_mandatory = True
    doc.save(ignore_permissions=True); frappe.db.commit()
    return {'success': True, 'name': doc.name}
...
def delete_record(doctype, record_name):
    allowed = ['Travel Lead','Booking','Tour Package','Visa Application','Destination','Visa Agent','Trip Run Sheet','Hotel Allotment','Supplier Contract','Cancellation Policy','Visa Country Config','Guide Allocation','Visa Delivery Log','Visa Fee Billing']
