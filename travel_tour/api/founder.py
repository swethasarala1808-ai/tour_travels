...
def get_founder_data():
    ...
    try:
        ...
        fname = (u.full_name or "Founder").split()[0]
    except Exception:
        fname = "Founder"
    return {
        "founder": {"full_name": fname, "first_name": fname},
        "stats":           _s(_stats),
        "monthly_revenue": _s(_monthly_revenue, []),
        "leads":           _s(_leads, []),
        "bookings":        _s(_bookings, []),
        "packages":        _s(_packages, []),
        "customers":       _s(_customers, []),
        "team":            [],
    }
...
def create_lead(**kwargs):
    ...
    if src not in valid_src:
        src = "Walk-in"
    valid_st = ["New","Contacted","Proposal Sent","Negotiation","Converted","Lost"]
...
def update_lead(**kwargs):
    ...
    try:
        ...
        valid_st = ["New","Contacted","Proposal Sent","Negotiation","Converted","Lost"]
        if kwargs.get("status") in valid_st:  doc.status = kwargs["status"]
        valid_src = ["Website","Social Media","Walk-in","Referral","Cold Call","Exhibition"]
...
def create_booking(**kwargs):
    ...
    try:
        ...
        dep   = kwargs.get("departure_date","").strip() or None
        frappe.db.sql("""
...
def update_booking(**kwargs):
    ...
    try:
        ...
        for k, col in [("tour_package","tour_package"),("grand_total","grand_total")]:
            if kwargs.get(k) is not None:
                sets.append(f"`{col}`=%s"); vals.append(kwargs[k])
        dep = kwargs.get("departure_date","").strip() if kwargs.get("departure_date") else None
        if dep:
            sets.append("`departure_date`=%s"); vals.append(dep)
        pax = cint(kwargs.get("total_pax") or kwargs.get("adult_pax",0))
        if pax:
            sets.append("`total_pax`=%s"); vals.append(pax)
        vals.append(bid)
...
def record_payment(**kwargs):
    ...
    try:
        frappe.get_doc("Booking", bname).add_comment("Comment",
            text=f"Payment \u20b9{amount:,.0f} via {kwargs.get('mode','\u2014')} "
